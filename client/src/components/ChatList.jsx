...
const ChatList = ({
  ...
  const loadCreateConversationOptions =
    useCallback(async () => {
      try {
        const [
          channelsRes,
        ...
        console.error(
          'Error loading conversation options:',
          error
        );
      }
    }, []);
  ...
  /*
   * Convert socket messages into conversation previews.
   *
   * IMPORTANT:
   * We use only conversationKey.
   */
  const enrichedChats =
    useMemo(() => {
      const chatMap =
      ...
      for (const [
        ...
        /*
         * New inbound message created a
         * new Conversation on the server.
         *
         * Show it immediately instead of
         * waiting for the 15-second refresh.
         */
        const separator =
          conversationKey.indexOf(
            '::'
          );

        const channelId =
          separator >= 0
            ? conversationKey.slice(
                0,
                separator
              )
            : '';
  ...
  const startConversation =
    async () => {
      ...
      try {
        ...
        // Add/open immediately.
        onSelectChat(
          conversation
        );

        // Refresh list so the conversation
        // appears permanently.
        await fetchChats();
    ...
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-full">
      <div className="p-4 bg-gray-50 border-b">
        ...
        <nav className="flex gap-1 mb-3">
          <button
            className="px-3 py-1 text-sm bg-green-100 text-green-700 rounded-md font-medium"
          >
            Chats
          </button>
